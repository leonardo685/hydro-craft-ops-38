...
import { Wrench, ClipboardList, Search, Calculator, CheckCircle, Receipt, Building2, ChevronUp, Users, Settings, LogOut, ChartBar, FileText, TrendingUp, CreditCard } from "lucide-react";
...
export function AppSidebar() {
  ...
  return <Sidebar className="border-r border-sidebar-border">
      <SidebarHeader className="p-6 border-b border-sidebar-border bg-slate-900">
          ...
          <SidebarGroupContent>
            <SidebarMenu>
              {menuItems.map(item => <SidebarMenuItem key={item.title}>
                  <SidebarMenuButton asChild isActive={location.pathname.startsWith(item.url)} className="transition-smooth hover:bg-sidebar-accent">
                    <button onClick={() => navigate(item.url)} className="flex items-center gap-3 w-full text-left">
      ...
      <SidebarFooter className="p-4 border-t border-sidebar-border bg-slate-900">
        <DropdownMenu>
