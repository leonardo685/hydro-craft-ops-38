...
import { Copy, Link2, CheckCircle2, Clock } from "lucide-react";
                      ...
                      return (
                        <TableRow key={it.id}>
                          ...
                          {forns.map((f) => {
                            ...
                            if (!p || p.preco_unitario == null)
                              return <TableCell key={f.id} className="text-right text-muted-foreground">—</TableCell>;
                            const isBest = melhor != null && Number(p.preco_unitario) === melhor;
                            return (
                              <TableCell
                                key={f.id}
                                className={`text-right ${isBest ? "bg-green-50 dark:bg-green-950/30 font-semibold" : ""}`}
                              >
