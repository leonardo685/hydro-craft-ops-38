...
export function ComprasKanban() {
        ...
        .select(`
          *,
          ordens_servico!inner (
            id, numero_ordem, cliente_nome, equipamento, status,
            pecas_necessarias, usinagem_necessaria, servicos_necessarios,
            recebimento_id, observacoes_tecnicas, updated_at,
            recebimentos ( cliente_nome, numero_ordem, tipo_equipamento )
          )
        `)
  ...
  useEffect(() => {
    if (empresaAtual?.id) loadCompras();
  }, [empresaAtual?.id]);
  ...
  const updateStatus = async (compraId: string, newStatus: "aprovado" | "cotando" | "comprado") => {
    try {
      ...
      const { error } = await supabase.from("compras").update(updates).eq("id", compraId);
      if (error) throw error;
  ...
  const renderColumn = (status: "aprovado" | "cotando" | "comprado", title: string) => {
    const comprasStatus = compras.filter((c) => {
      ...
      if (status === "comprado" && c.data_compra) {
        const diffHoras = (Date.now() - new Date(c.data_compra).getTime()) / 3600000;
        return diffHoras <= 24;
      }
      return true;
    });
